...
resource "azurerm_resource_group" "rg_azure_devops_agents" {
...
resource "azurerm_linux_virtual_machine_scale_set" "vmms" {
  ...
  location                        = azurerm_resource_group.rg_azure_devops_agents.location
  resource_group_name             = azurerm_resource_group.rg_azure_devops_agents.name
  sku                             = "Standard_B1s"
  instances                       = 1
  ...
  network_interface {
    ...
    ip_configuration {
      name      = "internal"
      primary   = true
      subnet_id = data.azurerm_subnet.snet_vmms.id
    }
  }
